...
ws_router = APIRouter()
SAMPLE_RATE = 16000
PRE_BUFFER_CHUNKS = 5  # ~160ms of lookback at 32ms/chunk — tune against real results
...
async def stream_transcribe(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            try:
                vad_result = shruti_engine.vad.process_pcm_chunk(vad_iterator, raw_chunk)
            ...
            if vad_result and "start" in vad_result:
                is_speaking = True
                # Seed the real buffer with whatever we already had in the
                # rolling pre-buffer, so we don't lose the first syllable to
                # VAD's detection lag — this is the streaming equivalent of
                # speech_pad_ms in batch mode.
                audio_buffer = list(pre_buffer)

            current_chunk = np.frombuffer(raw_chunk, dtype=np.int16)
            if is_speaking:
